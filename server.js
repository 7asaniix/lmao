...
const dev = [" 513058376092090393"];
const admin = "=";
var owner = "764626105943457822" ;"513058376092090393"; 
var number = "1"; 
var ex = "2019/10/08";
...
client.on("message", message => {
  if (message.content.startsWith("=new")) {
    ...
    if (!message.guild.roles.exists("name", "Support Team"))
      return message.channel.send(` you have to make a role named \`Support Team\`.`);
    if (
...
client.on("message", zaid => {
  if (zaid.content === "=bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@513058376092090393>`, true) // تعديل مهم عدل هذا الرقم لايدي حسابك
      .setImage("")
...
client.on("message", async message => {
  if (message.content.startsWith(prefix + "inf")) {
    ...
    message.guild.fetchInvites().then(invs => {
      ...
      let possibleInvites = [["Total number of people"]];
...
client.on("message", async msg => {
  ...
  if (msg.content.startsWith("link")) {
    ...
    msg.author.send(
      invite
        ? /*seccussfull*/ `**the link time is only for one day and eligible to invite 5 people only **:\n ${invite}` /*error catch*/
        : "there is an error in the bot please open a ticket in th server of the person who made the bot and tell him ..."
    );
  }
});
...
client.on("message", async msg => {
  ...
  if (msg.content.startsWith(prefix + "setstats")) {
    ...
    if (!msg.guild.member(client.user).hasPermission(["MANAGE_CHANNELS"]))
      return msg.reply("❌ **the bot does not have perms**");
...
client.on("message", message => {
  ...
  if (command === "=unmute") {
    if (!message.member.hasPermission("MANAGE_ROLES"))
      return message
        .reply("** you dont have perms 'Manage Roles' **")
        .catch(console.error);
    ...
    if (!muteRole)
      return message
        .reply("** there is no role  -> 'Muted' **")
        .catch(console.error);
    if (message.mentions.users.size < 1)
      return message
        .reply("** mention a person**")
        .catch(console.error);
    const embed = new Discord.RichEmbed()
      .setColor(0x00ae86)
      .setTimestamp()
...
client.on("message", message => {
  ...
  if (message.content === prefix + "1help") {
    message.channel.send(
      `**:white_check_mark:  | general commands are sent on dm **`
    );
...
**  =avt • give avatar using id** 
**  =avatar • gives avatar using mention** 
**  =color • change color **
...
client.on("message", message => {
  ...
  if (message.content === prefix + "2help") {
    message.channel.send(
      `**:white_check_mark:  |admin commands are sent in the direct messages **`
    );

    message.author.sendMessage(` **✽ Premium Bot v1**
   **__ADMIN COMMANDS__** ✽ 
:gem: **  =clear • clears chat** 
:gem: **  =ban • to ban someone** 
:gem: **  =kick • to kick someone** 
:gem: **  =unmute • unmute someone** 
:gem: **  =new •  open support ticket**
:gem: **  =move •  to move someone to your vc room **
:gem: **  =giveaway •  create a giveaway**
...
client.on("message", message => {
  ...
  if (message.content === prefix + "3help") {
    ...
    message.author.sendMessage(`  **Premium Bot v1**
  **__music commands__**   
** temprorarily this bot does not support musinc commands **
...
client.on("message", message => {
  ...
  if (message.content === prefix + "4help") {
    ...
    message.channel.send(
      `**:white_check_mark:  | secruity commands are sent om dm **`
    );
...
client.on("message", message => {
  ...
  if (message.content === prefix + "5help") {
    message.channel.send(
      `**:white_check_mark: | special commands are sent on dm **`
    );

    message.author.sendMessage(` **Premium Bot v1**

** =creatcolores • to create colors **
** =colors • change your color ** 
** =inf • how many people you invited to the server**
** =credits • check your credits ** 
** =daily • claim your daily money**
** =removecredits • delete credits (only for yourself not for other people) **
** =addcredits • to add credit to other people **
** example: \`=addcredits 500 \`**
** =setLog • set log room**
** =setby • bye bye room**
`);
  }
...
> ** =1help ** ** general commands  ** 
> ** =2help ** ** admin commands  **
> ** =3help ** ** music commands **
> ** =4help ** ** owner commands  **
> ** =5help ** ** special commands **
 **__Done__** 
   `);
  }
});
  ...
  if (message.content === prefix + "creatcolores") {
    ...
    message.channel.sendMessage({
      embed: new Discord.RichEmbed()
        ...
        .setDescription("``colors are under construction....``")
    });
  }
});
...
client.on("message", ra3d => {
  ...
  if (ra3d.content.startsWith(prefix + "ccolors")) {
    if (!args) return ra3d.channel.send("`choose color `");
    if (!ra3d.member.hasPermission("MANAGE_ROLES"))
      return ra3d.channel.sendMessage(
        "`**⚠ | `[MANAGE_ROLES]` you dont have perms**"
      );
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "giveaway")) {
    //return message.channel.send('**there is some problems ; try later**');
    if (!message.guild.member(message.author).hasPermission("MANAGE_GUILD"))
      return message.channel.send(
        ":heavy_multiplication_x:| **you dont have manage server perms**"
      );
    message.channel
      .send(`**write the room name without mention ( # )**`)
      .then(msgg => {
        message.channel
          ...
          .then(collected => {
            ...
            if (!room)
              return message.channel.send(
                "**i cant find room | try again later**"
              );
            room = collected.first().content;
            collected.first().delete();
            msgg.edit("**write time in minutes**").then(msg => {
              message.channel
                .awaitMessages(filter, {
                  max: 1, //HactorMC
                  time: 20000,
                ...
                .then(collected => {
                  if (isNaN(collected.first().content))
                    return message.channel.send(
                      ":heavy_multiplication_x:| **write correct time command``**"
                    );
                  duration = collected.first().content * 60000;
                  collected.first().delete();
                  msgg
                    .edit(
                      ":eight_pointed_black_star:| **the givaway is for what?**"
                    )
                    .then(msg => {
                      message.channel
                        .awaitMessages(filter, {
                        ...
                        .then(collected => {
                          ...
                          try {
                            ...
                            msgg.edit(
                              `:heavy_check_mark:| **giveway setup done**`
                            );
                          } catch (e) {
                            msgg.edit(
                              `:heavy_multiplication_x:| **i cant make giveway because i dont have perms**`
                            );
                            console.log(e);
                          }
                        });
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    if (
      ...
      return message.channel.send("**u dont have perms . ❌**");
    message.channel.send(`**${args} goodbye setup done :loudspeaker: **`); //By ItzTexo
    client.on("guildMemberAdd", member => {
      ...
      var embed = new Discord.RichEmbed()
        ...
        .setTitle(`**GOODBYEEE**`)
        .addField("**__Thanks for your time__**  ", `${member}`)
        .setDescription(`**pls come back soon ✋😢** `)
...
client.on("message", async message => {
  ...
  if (cmd === `${prefix}kick`) {
    ...
    if (!kUser) return message.channel.send("where is the member?");
    let kReason = args.join(" ").slice(22);
    if (!message.member.hasPermission("MANAGE_CHANNELS"))
      return message.channel.send("you dont have perms");
    if (kUser.hasPermission("MANAGE_CHANNELS"))
      return message.channel.send("lmao you cant kick admins");
    ...
    if (!kickChannel) return message.channel.send("i did not find the kick-ban room");
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "settings limits")) {
    if (message.author.id !== message.guild.owner.user.id)
      return message.channel.send(`**you cant use this commands**`);
    if (message.content.startsWith(prefix + "settings limitsban")) {
      if (!num) return message.channel.send("**⇏ | send a number ! **");
      if (isNaN(num)) return message.channel.send("**⇏ | onlyyyy numbers ! **");
    ...
    if (message.content.startsWith(prefix + "settings limitskick")) {
      if (!num) return message.channel.send("**⇏ |send a  number ! **");
      if (isNaN(num)) return message.channel.send("**⇏ | onlyyyyy numbers ! **");
    ...
    if (message.content.startsWith(prefix + "settings limitsroleD")) {
      if (!num) return message.channel.send("**⇏ | send a number ! **");
      if (isNaN(num)) return message.channel.send("**⇏ | onlyyyyyy numbers ! **");
    ...
    if (message.content.startsWith(prefix + "settings limitsroleC")) {
      if (!num) return message.channel.send("**⇏ | send a nmber ! **");
      if (isNaN(num)) return message.channel.send("**⇏ | onlyyyy numbers ! **");
      ...
      channel.guild.members
        ...
        .catch(e =>
          channel.guild.owner.send(
            `**⇏ | ${entry.username}  deleted many channels **`
          )
        );
      anti[channel.guild.id + entry.id].actions = "0";
  ...
  if (!anti[channel.guild.id + entry.id]) {
    ...
    if (
      ...
      channel.guild.members
        ...
        .catch(e =>
          channel.guild.owner.send(
            `**⇏ | ${entry.username} deleted many roles **`
          )
        );
      anti[channel.guild.id + entry.id].actions = "0";
      fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
  ...
  if (!anti[channel.guild.id + entry.id]) {
    ...
    if (
      ...
      channel.guild.members
        ...
        .catch(e =>
          channel.guild.owner.send(
            `**⇏ | ${entry.username} made many roles **`
          )
        );
      anti[channel.guild.id + entry.id].actions = "0";
      fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
  ...
  if (!anti[guild.id + entry.id]) {
    ...
    if (anti[guild.id + entry.id].actions >= config[guild.id].banLimit) {
      guild.members
        ...
        .ban()
        .catch(e =>
          guild.owner.send(`**⇏ | ${entry.username} tried to nuke the server**`)
        );
      anti[guild.id + entry.id].actions = "0";
      fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
        e
    ...
    if (!anti[member.guild.id + entry.id]) {
      ...
      if (
        ...
        member.members
          ...
          .catch(e =>
            member.owner.send(
              `**⇏ | ${entry.username} tried to nuke the server **`
            )
          );
        anti[member.guild.id + entry.id].actions = "0";
        fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
...
tried to hack the server @everyone`);
    channel.guild.owner.send(`<@!${channelcreate.id}>
tried to hack the server ${channel.guild.name}`);
  }
  setTimeout(() => {
